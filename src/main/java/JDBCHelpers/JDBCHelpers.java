...
public class JDBCHelpers {
    ...
    public static java.sql.Connection getConnection(@NotNull String url, @NotNull String username, @NotNull String password, @Nullable String database) {
        try{
            //TODO Add support for other drivers by changing the static jdbc:mysql: string to some object
            if(database != null) return DriverManager.getConnection("jdbc:mysql://"+url+"/"+database, username, password);
            else return DriverManager.getConnection("jdbc:mysql://"+url, username, password);
    ...
    public static java.sql.Connection getConnection(@NotNull String url, @NotNull String username, @NotNull String password) {
        return getConnection(url, username, password, null);
    }

    public static ResultSet getScrollableResultSet(@NotNull java.sql.Connection connection, @NotNull String sqlQuery){
        try{
    ...
    public static boolean closeConnection(@NotNull java.sql.Connection connection) {
        try{
            connection.close();
            return true;
        }catch (SQLException e){
